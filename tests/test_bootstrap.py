...
class BootstrapTests(unittest.TestCase):
    ...
    def test_bootstrap_master_apply_writes_files_and_runs_runtime_commands(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("/etc/sysctl.d/99-linkray-network.conf" in command for command in runner.commands))
            self.assertTrue(any("modprobe tcp_bbr" in command for command in runner.commands))
            self.assertTrue(any("tc qdisc replace dev" in command for command in runner.commands))
    ...
    def test_bootstrap_node_apply_writes_host_services(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(any("get.docker.com" in command for command in runner.commands))
            self.assertFalse(any("systemctl enable --now docker" in command for command in runner.commands))
            self.assertFalse(any("docker compose up" in command for command in runner.commands))
