    ...
    def test_dashboard_patch_injects_node_info_panel(self):
        for patch_path in [
            ...
            with self.subTest(patch=str(patch_path)):
                ...
                self.assertIn("Snell 专用链路", patch)
                self.assertIn("base+'/shadowrocket-conf'", patch)
                self.assertNotIn("Egern Snell", patch)
                self.assertIn("Egern 可使用 Clash/Mihomo Snell", patch)
    ...
    def test_subscription_page_lists_auto_first_and_no_clash_for_windows(self):
        for page_path in [
            ...
            with self.subTest(page=str(page_path)):
                ...
                self.assertIn("Snell 专用链路", html)
                self.assertIn("base + '/shadowrocket-conf'", html)
                self.assertNotIn("Egern Snell", html)
                self.assertIn("Egern 可使用 Clash/Mihomo Snell", html)
