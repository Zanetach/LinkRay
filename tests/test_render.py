...
class RenderTests(unittest.TestCase):
    ...
    def test_render_master_writes_expected_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Starting Marzban nodes Xray core", xray_core_patch)
            self.assertIn("xray.operations.connect_node(node_id, config)", xray_core_patch)
            self.assertNotIn("Skipping Marzban nodes Xray core", xray_core_patch)
            ...
            self.assertIn("location /linkray/rules/", nginx)
            self.assertIn("alias /var/lib/marzban/linkray/rules/", nginx)
            ...
            self.assertNotIn("--snell-runtime-dir", clash_service)
            self.assertNotIn("--snell-reload-command", clash_service)
            ...
            self.assertNotIn("--snell-runtime-dir", egern_service)
            self.assertNotIn("--snell-reload-command", egern_service)
    ...
    def test_dashboard_patch_injects_node_info_panel(self):
        for patch_path in [
            ...
            with self.subTest(patch=str(patch_path)):
                ...
                self.assertIn("https://sing-box.sagernet.org/clients/", patch)
                self.assertIn("https://apps.apple.com/app/shadowrocket/id932747118", patch)
                self.assertIn("https://apps.apple.com/app/egern/id1616105820", patch)
    ...
    def test_subscription_page_lists_auto_first_and_no_clash_for_windows(self):
        for page_path in [
            ...
            with self.subTest(page=str(page_path)):
                ...
                self.assertIn("sing-box 高级链路", html)
                self.assertNotIn("Snell 专用链路", html)
                ...
                self.assertIn("https://sing-box.sagernet.org/clients/", html)
    ...
    def test_dashboard_html_adds_protocol_inbound_details_without_touching_app_bundle(self):
        for html_path in [
            ...
            with self.subTest(html=str(html_path)):
                ...
                self.assertIn("document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT", html)
                self.assertIn("function retargetStarEntry()", html)
                self.assertIn("retargetStarEntry();", html)
                self.assertIn("https://github.com/Zanetach/LinkRay", html)
    ...
    def test_hosts_sql_contains_two_node_rows(self):
        ...
        self.assertIn("INSERT OR IGNORE INTO nodes", sql)
        self.assertIn("'edge-b', 'edge-b.example.com', 62050, 62051, 'connecting'", sql)
        self.assertNotIn("'edge-a', 'edge-a.example.com', 62050", sql)
