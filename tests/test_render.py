    ...
    def test_dashboard_patch_injects_node_info_panel(self):
        for patch_path in [
            ...
            with self.subTest(patch=str(patch_path)):
                ...
                self.assertIn("['Shadowrocket 配置',base+'/shadowrocket-conf']", patch)
                self.assertIn("['Shadowrocket 节点',base+'/shadowrocket']", patch)
    ...
    def test_subscription_page_lists_auto_first_and_no_clash_for_windows(self):
        for page_path in [
            ...
            with self.subTest(page=str(page_path)):
                ...
                self.assertIn("['Shadowrocket 配置', base + '/shadowrocket-conf']", html)
                self.assertIn("['Shadowrocket 节点', base + '/shadowrocket']", html)
                self.assertLess(html.index("['自动识别订阅', base]"), html.index("['Clash/Mihomo', base + '/clash-meta']"))
                self.assertLess(html.index("['Clash/Mihomo', base + '/clash-meta']"), html.index("['Shadowrocket 配置', base + '/shadowrocket-conf']"))
